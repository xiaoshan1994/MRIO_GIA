...
##############################################
##############################################
#Load Exiobase V3.8.1(2020), monetary IOT, product by product
...
# lists of value added components, co2-related, and water-consumption-related indicators. 
...
pos_co2 = labels_co2_pd.index      

labels_wc_pd = labels_ext_pd[labels_ext_pd.Name.str.contains('water consumption', case=False)]        # filter by extention Name, including 'water consumption'
pos_wc = labels_wc_pd.index                                                              # position of the water-consumption-related extension indicators
...
F_co2 = F[pos_co2,:]
F_co2 = np.sum(F_co2,0)   

F_wc = F[pos_wc,:]
F_wc = np.sum(F_wc,0)                                                                # obtain a CO2 extension vector 9800 cols
...
label = {'region': labels_reg_pd, 'product': labels_prod_pd, 'final': labels_final_pd, 'va': labels_va_pd, 'co2': labels_co2_pd, 'wc':labels_wc_pd}
mrio_y2020 = {'F': F, 'H': H, 'V': F_va, 'Y': Y, 'L': L, 'F_co2': F_co2, 'H_co2': H_co2, 'F_wc': F_wc, 'H_wc': H_wc,'label': label}

#############################################
# save to pickle
mrio_name = 'mrio_y2020.pkl'    
mrio_str = os.path.abspath('OneDrive - Universiteit Leiden/4. Leiden Univ/2021-WN EIOA course by Ranran/IGA/mrio_y2020.pkl')                                                    # .pkl: a Python file type
pkl_out = open(mrio_str,"wb")                                                              # 'open' the created mrio.pkl file
pkl.dump(mrio_y2020, pkl_out)                                                                    # 'dump' the mrio data to mrio.pkl
pkl_out.close()
